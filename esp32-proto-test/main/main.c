...
uint8_t message_with_pointer(struct _Test__Message * message)
{
    ...
    char * test_ptr;
    test_ptr = (char *)malloc(sizeof(test));
    memcpy(test_ptr, test, sizeof(test));
    message->a = a;
    message->b = b;
    //message->test = test;
    message->test = test_ptr;
    return 0;
}
...
int test(void)
{
  ...
  message_with_pointer(&message);
  //assert (message.a == 32);
  printf("message.a is      : %i\n", message.a);
  printf("message.test is   : %s\n", message.test);
  printf("sizeof message.test: %i", (sizeof(message.test) * sizeof(message.test[0])));
